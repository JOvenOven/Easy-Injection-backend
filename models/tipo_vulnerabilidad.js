...
class VulnerabilityType {
    ...
    async save() {
        if (this._id) {
            ...
            if (!updated) {
                throw new Error(`VulnerabilityType with _id ${this._id} not found`);
            }
    ...
    static async find(query = {}) {
        ...
        return docs.map(doc => new VulnerabilityType(doc.toObject()));
    }

    static async findOne(query) {
        const doc = await VulnerabilityTypeModel.findOne(query);
        return doc ? new VulnerabilityType(doc.toObject()) : null;
    }
    ...
    static async create(data) {
        ...
        const saved = await doc.save();
        return new VulnerabilityType(saved.toObject());
    }
